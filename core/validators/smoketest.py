...
# Add parent directories to path to support running from validators/ directory
_file_path = Path(__file__).resolve()
_core_path = _file_path.parent.parent
...
int f() {
    ...
    TestCase("sv_chain1_step3_string_view", # does not work on MSVC
    ...
    TestCase("span_chain1_step2_to_span", # does not work on MSVC
...
def print_summary(total_passed, total_failed):
    ...
    print("=" * 60)
    print(f"Validator tests: {total_passed:3d} passed, {total_failed:3d} failed")
    print("-" * 60)
...
if __name__ == "__main__":
    ...
    passed, failed = run_validator_smoke_tests(compilers)
